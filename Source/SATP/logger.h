...
#ifndef SATP_LOGGER_H
#define SATP_LOGGER_H

#include "satpcommon.h"

/**
 * \file logger.h
 * \brief SATP logging functions.
 *
 * \details
 * This header file defines the internal logging functions for the Quantum Secure Messaging Protocol (SATP).
 * The logging subsystem provides basic functionality to create, manage, and inspect log files. These functions
 * allow the SATP system to record operational events, errors, and other informational messages which can be used
 * for debugging and monitoring purposes.
 *
 * The following constants are defined:
 *
 * - \c SATP_LOGGING_MESSAGE_MAX: Maximum length allowed for a single log message.
 * - \c SATP_LOGGER_PATH: The default directory path for SATP log files.
 * - \c SATP_LOGGER_FILE: The default filename for the SATP log.
 * - \c SATP_LOGGER_HEAD: The header string for the log file, which typically includes version information.
 *
 * \note These functions and constants are internal and non-exportable.
 */

/**
 * \def SATP_LOGGING_MESSAGE_MAX
 ...
 */
#define SATP_LOGGING_MESSAGE_MAX 256U

/**
 * \var SATP_LOGGER_PATH
 * \brief Default directory path for SATP log files.
 *
 * This static constant defines the default directory where the SATP log file is stored.
 */
static const char SATP_LOGGER_PATH[] = "SATP";

/**
 * \var SATP_LOGGER_FILE
 * \brief Default log file name.
 *
 * This static constant defines the default name of the SATP log file.
 */
static const char SATP_LOGGER_FILE[] = "satp.log";

/**
 * \var SATP_LOGGER_HEAD
 ...
 *
 * \details
 * This function checks for the existence of the SATP log file in the configured logging directory.
 *
...
/**
 * \brief Initialize the SATP logger.
 *
 * \details
 * This function initializes the logging subsystem by setting the log file path and creating the log file if it does not
 * already exist. The default header (\c SATP_LOGGER_HEAD) is written to the log file upon initialization.
 *
...
/**
 * \brief Print the contents of the SATP log file.
 *
 * \details
 * This function outputs the entire contents of the SATP log file to the standard output or designated debug stream.
 * It is useful for real-time monitoring and debugging purposes.
 */
void satp_logger_print(void);

/**
 * \brief Read the SATP log file into a provided buffer.
 *
 * \details
 * This function reads the content of the SATP log file and copies it into the specified output buffer.
...
/**
 * \brief Reset the SATP log.
 *
 * \details
 * This function erases all the contents of the SATP log file, effectively resetting it to an empty state.
 * This operation is useful for clearing old log data before starting a new session.
 */
void satp_logger_reset(void);

/**
 * \brief Get the current size of the SATP log file.
 *
 * \details
...
/**
 * \brief Write a message to the SATP log file.
 *
 * \details
 * This function writes the specified log message to the SATP log file. The message should be a null-terminated string,
 * and its length should not exceed \c SATP_LOGGING_MESSAGE_MAX characters.
 *
...
/**
 * \brief Run a manual test of the SATP logger functions.
 *
 * \details
 * This function performs a series of tests on the SATP logging subsystem. The test routine includes:
 *
